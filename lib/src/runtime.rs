...
/// Run blocking work (e.g. model inference) from async code without stalling the
/// executor. On a multi-thread runtime this is `block_in_place`, which relocates
/// other tasks off the worker for the duration; otherwise (no runtime, or a
/// current-thread runtime that has no worker to free) it just runs the closure.
/// Either way it never panics and never blocks sibling tasks on a live
/// multi-thread executor (RT-1).
