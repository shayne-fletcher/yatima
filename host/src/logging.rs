//! File logging behind `$YATIMA_LOG` — the one twin the TUI and GUI each kept.
//!
//! OBS-1: the lib emits spans/events, the host decides where they go. Every
//! frontend owns its screen (ratatui's terminal, egui's window), so logs never
//! go to stdout/stderr; they append to a per-host file under `~/.cache/yatima`.
