...
(DeepSeek is safetensors-only — candle has no quantized DeepSeek loader). A CI
consistency harness checks every family's wiring (detection, GGUF normalization,
chat-format mapping). The agent/tool path is narrower by design: it needs a model
trained to emit tool calls (today: Qwen/ChatML).

`‡` marks families that are **wired + harness-tested but not yet
runtime-validated with weights** — loading and generation are unverified on a
real model of that family.
