...
- **Engine swappability** — cross-arch dispatch + GGUF/quantized + self-contained
  ...
  impls.
- **Local llama.cpp backend — Stage 2 complete in the working tree (2026-08-22).** Yatima implements `Completer` over llama-server's raw `/completion` endpoint and can either attach to a validated loopback origin or own the child process. The managed path resolves one exact GGUF, starts `llama-server` with pinned loopback and single-slot arguments, bounds readiness and introspection, drains both output pipes, races in-flight completion against child death, and kills, reaps, and joins on explicit shutdown and every child-owning startup failure (LSRV-1/2, MS-4, CANCEL-1). The division is deliberate: llama.cpp owns model execution, architecture support, quantization, Metal acceleration, tokenization, and KV caches; Yatima owns prompt rendering, transcripts, reasoning and tool protocols, capabilities, agent control flow, invariants, and frontends. Keeping the boundary at raw prompts and streamed text prevents `/v1/chat/completions` from becoming a second owner of chat or tool meaning; `/apply-template` remains a fixture oracle while Yatima's renderer stays authoritative. `ModelSource` and possum select and acquire the exact artifact, and the managed server receives that canonical GGUF with `-m`; `llama-server -hf` remains a manual convenience rather than the product path because it would create a second cache and bypass Yatima's source and offline rules. Generated self-description and `/props` remain untrusted identity claims. Stage 2 records the launched artifact but labels it unverified; Stage 3 may name a verified Muse model only after checking the profile-pinned digest, build, and embedded template (LSRV-5). Candle remains the in-process backend. Next: finish Muse's live ATEM classification and profile; vision, DFlash, tools, host integration, and in-process llama.cpp FFI remain later work.
