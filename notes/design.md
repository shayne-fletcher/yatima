...
- **GUI Glimmer visual pass (after stage 5).** First integrate the verified Muse Glimmer backend and tool-capable path into `yatima-gui`. Then capture representative egui chat and tool-round screenshots and revisit the in-app art against those real screens. Do not redesign the art before the backend and interaction states exist.
