...
- **`yatima-gui`** — the GPU frontend (egui/eframe, wgpu → Metal): the same thin
  ...
  Decorative animation is behind `--whimsy` (off by default). The binary retains the one `HostOwner`; `GuiApp` receives only `HostClient`, folds typed startup phases into one loading/ready/failed state, and shows verified identity only when carried by `ModelInfo`.
...
- **Local llama.cpp backend — shared host and frontend integration implemented (2026-08-30).** The host backend thread owns either Candle or a verified managed `llama-server`, while `HostClient` carries movable request/event/cancel planes and `HostOwner` carries the joined-shutdown proof. The TUI, native GUI, and serve/browser path accept `--profile muse-glimmer`, show typed startup phases and verified identity, and use the same chat/tool event meanings. TUI live acceptance is complete; GUI and browser live acceptance remain the final Stage-5c gate before this integration arc closes.
- **GUI Glimmer visual subtraction (after stage 5).** First integrate the verified Muse Glimmer backend and tool-capable path into `yatima-gui`. Then remove `--whimsy` and its animation-only state, timing, drawing branches, and continuous repaint machinery: it proved egui's range, but no longer pays its complexity against Yatima's quiet, functional, minimal character. Preserve state-bearing visuals and the low-frequency loading-clock repaint. Against representative live chat and tool-round screens, evaluate `images/yatima-2.png` as one restrained loading/empty-state artifact rather than replacing whimsy with another decorative system; capture screenshots only after that comparison.
