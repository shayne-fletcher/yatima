...
`yatima` is a small Rust runtime for **language-integrated LLMs**: call a local model from ordinary Rust and let it act only through explicit capabilities. It is a building block rather than one fixed application: embed it, wrap it in a service, or drive it from a native frontend. Yatima owns prompts, transcripts, tool protocols, capabilities, and agent control flow. Model execution can run in-process through [Candle](https://github.com/huggingface/candle) or through a supervised local [llama-server](https://github.com/ggml-org/llama.cpp).
...
- **`yatima-web`** — that browser client (the vision's rung 2): an
  ...
  It folds startup, ready, and fatal events into one backend state, disables input until both connected and ready, and displays only verified identity evidence. Depends on `yatima-protocol` + `yatima-text` alone, WASM-clean by construction.
...
- **Image-action adherence and session display memory — complete (2026-09-06).** The founding three-turn Mandelbrot tape proved that the host and views delivered every artifact they received; the model's third reply merely narrated a future `read_image` call, while 12 of the first 35 display events repeated bytes from an earlier turn. `ReadImage` now projects its session memo into each regenerated tool spec as compact already-shown/not-yet-shown number ranges, and duplicate URL or content-hash calls emit no display event unless `again: true` records an explicit re-show request. A tool may also declare a narrow, deterministic call requirement over the current user text. For explicit image show/display/render/fetch requests, `Agent` withholds and retracts any candidate final answer until `read_image` succeeds; every retry consumes the same finite AGENT-1 budget, and a fabricated effect enters neither `Final` nor history. The after tape (`runs/20260906T153426Z-66346-drive`) records the former failure occurring and being corrected: turn 3 retracts 976 characters, records the unmet requirement, then invokes `read_image` for eight images. The completed session emits 6/7/8 images by turn, with 21 events and 21 unique SHA-256 digests (AGENT-1/4, IMG-2/3).
- **Image identity follows the pixels — complete (2026-09-06).** The same tape exposed a second host-side loss: `read_image` selected numbered, captioned entries, but `ToolEvent::Artifact` reduced each one to a hash-named path before any view or recorder saw it. `ToolArtifact` now carries the path together with the source URL, current list number, and label. The agent, host protocol, native GUI, TUI, browser mirror, CLI, and flight recorder preserve that record. Inline views render a linked `image N · label` caption, the terminal posts the same identity after the turn, and tapes retain it beside the out-of-line bytes. The proof run (`runs/20260906T162215Z-70202-drive`) completed the founding three turns with 6/7/8 images: every one of its 21 unique artifacts has a label, source, and list number, and those numbers exactly match the `read_image` arguments. Model prose is no longer responsible for explaining which pixels are which (IMG-2/3).
...
- **Engine swappability** — cross-arch dispatch + GGUF/quantized + self-contained
  ...
  impls.
- **Local llama.cpp backend — managed Muse chat complete (2026-08-23).** Yatima implements `Completer` over llama-server's raw `/completion` endpoint and can either attach to a validated loopback origin or own the child process. `yatima chat --profile muse-glimmer` resolves one exact GGUF, verifies the profile-pinned digest, launches that canonical path, gates the server build, embedded template, context, and slot count, classifies streamed ATEM reasoning separately from answers, commits answer-only history, and reaps the child on exit (LSRV-1/2/5, MS-4, REASON-1, CANCEL-1). The division is deliberate: llama.cpp owns model execution, architecture support, quantization, Metal acceleration, tokenization, and KV caches; Yatima owns prompt rendering, transcripts, reasoning and tool protocols, capabilities, agent control flow, invariants, and frontends. Keeping the boundary at raw prompts and streamed text prevents `/v1/chat/completions` from becoming a second owner of chat or tool meaning; `/apply-template` remains a fixture oracle while Yatima's renderer stays authoritative. `ModelSource` and possum select and acquire the exact artifact; `llama-server -hf` remains a manual convenience rather than the product path because it would create a second cache and bypass Yatima's source and offline rules. Generated self-description and attached `/props` remain untrusted identity claims. Candle remains the in-process backend. Vision, DFlash, `/tokenize`, and in-process llama.cpp FFI remain later work.
...
- **GUI Glimmer visual subtraction — complete (2026-08-30).** `--whimsy` and everything behind it — the avatar, easter eggs, splash, status ticker, and the always-on repaint machinery — are removed at the maintainer's direction ("it's cute but it code for nothing"). What remains is state-bearing: the /stats grid, typed startup phases and verified identity, streaming turn displays, and a one-second repaint only while loading (the help overlay, animated at first, became a still page at the maintainer's direction on 2026-09-04: "just an accurate useful help listing is more in character"). It proved egui's range and then stopped paying for its complexity; no decorative replacement is planned.
...
- **Speculative decoding over the wire (idea, 2026-07-12).** Provenance in
  ...
  *batched* confirmation queries, verifies up to token N, and returns the
  ...
  small-model drafting degrades gracefully and fits the same protocol. Far
  out of spike scope; recorded because the serve seam is where it would
  land.
